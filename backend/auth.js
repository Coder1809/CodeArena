  ...
  try {
    ...
    const insertRes = await db.query(
      `INSERT INTO users (username, email, password_hash, cf_handle)
       VALUES ($1, $2, $3, $4)
       RETURNING id, username, email, cf_handle, wins, losses`,
...
router.post('/login', async (req, res) => {
  try {
    ...
    const result = await db.query(
      `SELECT id, username, email, cf_handle, wins, losses
...
router.get('/me', async (req, res) => {
  ...
  try {
    ...
    const userRes = await db.query(
      `SELECT id, username, email, cf_handle, wins, losses FROM users WHERE id = $1`,
      [decoded.id]
    );
...
router.post('/update-cf', async (req, res) => {
  ...
  try {
    ...
    const updated = await db.query(
      `UPDATE users SET cf_handle = $1 WHERE id = $2 RETURNING id, username, email, cf_handle, wins, losses`,
